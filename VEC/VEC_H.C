...
struct definition {
    ...
    {0, "VPSXV%d(to,v,s,w)",  "(to)[%i] = (v)[%i] + (s) * (w)[%i]",    ","},
    {0, "VPVXS%d(to,v,w,s)",  "(to)[%i] = (v)[%i] + (w)[%i] * (s)",    ","},
    ...
    {0, "VMODS%d(to,v,s)",    "(to)[%i] = SMODS1((v)[%i], s)",             ","},
    {0, "VMODV%d(to,v0,v1)",  "(to)[%i] = SMODS1((v0)[%i], (v1)[%i])",     ","},
    {0, "VDIVS%d(to,v,s)",    "(to)[%i] = SDIVS1((v)[%i], s)",     ","},
    {0, "VDIVV%d(to,v0,v1)",  "(to)[%i] = SDIVS1((v0)[%i], (v1)[%i])",     ","},
    {0, "VMINS%d(to,v,s)",    "(to)[%i] = SMINS1((v)[%i], s)",     ","},
    {0, "VMINV%d(to,v0,v1)",  "(to)[%i] = SMINS1((v0)[%i], (v1)[%i])",     ","},
    {0, "VMAXS%d(to,v,s)",    "(to)[%i] = SMAXS1((v)[%i], s)",     ","},
    {0, "VMAXV%d(to,v0,v1)",  "(to)[%i] = SMAXS1((v0)[%i], (v1)[%i])",     ","},
  ...
  "VXM%d-1((to%d)[%i],(m%d)[%i],m%d-1), (to%d)[%i][%d-1]=(m%d)[%i][%d-1]", ","},
    ...
    {0, "SMODS1(a,b)",  "(((a)%%(b)+(b))%%(b))"},
    {0, "SDIVS1(a,b)",  "(((a)-SMODS1(a,b))/(b))"},
    {0, "SMINS1(a,b)",  "((a) < (b) ? (a) : (b))"},
    {0, "SMAXS1(a,b)",  "((a) > (b) ? (a) : (b))"},
    ...
    {0, "__ADJOINTcol%d(to,col,m,i%1..d-1)",
     "(to)[%i][col] = %--_DET%d-1(m[i%1..d-1], %~i)", ","},
    /*
     * Inverse-- completely special case, defined in terms of adjoint and det
     */
    {2, "INVERTMAT%d(to,from)"}
};
 ...
 */
void printformatted_to(s, tochars, d, i)
char *s, *tochars;
int d, i;
{
    char *prefix, *suffix;
...
void print_second_arg(name_and_arg, d)
char *name_and_arg;
int d;
{
    printformatted_to(strchr(name_and_arg, ',') + 1, ",)", d, 0);
...
char **argv;
{
 ...
 for (i = 0; i < numberof(defs); ++i) {
  ...
  continue; /* don't redefine if it's the same */
     if (!defs[i].all_but_last)
  continue; /* no definition here */
 ...
 for (i = 0; i < numberof(defs); ++i) {
  ...
  if (isprefix("INVERTMAT", defs[i].name_and_args)) {
      printformatted("ADJOINT%d(_vec_h_temp_, ", d, 0);
      print_second_arg(defs[i].name_and_args, d);
      printf("); \\\n\t\t    ");
      printformatted("type _vec_h_temp_invdet_ = (type)1/(type)DET%d(from); \\\n\t\t    ", d, 0);
      printformatted("SXM%d(", d, 0);
      print_first_arg(defs[i].name_and_args, d);
      printf(", _vec_h_temp_invdet_");
  } else {
